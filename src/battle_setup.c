...
//To be Expanded
static bool8 ShouldUseForestTerrain(void){
    u16 mapSection = getCurrentMapSection();
...
u8 BattleSetup_GetTerrainId(void)
{
    ...
    //if (ShouldUseIceTerrain() && MetatileBehavior_IsTallGrass)
    //    return BATTLE_TERRAIN_ICE_GRASS;

    if(DisableSeasonalPalette())
    ...
    switch (gMapHeader.mapType)
    {
        ...
        case MAP_TYPE_UNDERGROUND:
            if(ShouldUseForestTerrain() && !MetatileBehavior_IsSurfableWaterOrUnderwater(tileBehavior))
        ...
        case MAP_TYPE_OCEAN_ROUTE:
            if (MetatileBehavior_IsSurfableWaterOrUnderwater(tileBehavior))
                ...
                switch(season){
                    case SEASON_AUTUMN:
                        return BATTLE_TERRAIN_PLAIN_AUTUMN;
                    break;
                    case SEASON_WINTER:
                        return BATTLE_TERRAIN_PLAIN_WINTER;
                    break;
                    default:
    ...
    if (MetatileBehavior_IsTallGrass(tileBehavior))
    {
        switch(season){
            ...
            /*case SEASON_SUMMER:
                if(time == TIME_SUNSET)
                    return BATTLE_TERRAIN_GRASS_SUMMER_DUSK;
                else if(time == TIME_NIGHT)
                    return BATTLE_TERRAIN_GRASS_SUMMER_NIGHT;
                else
                    return BATTLE_TERRAIN_GRASS_SUMMER;
            break;*/
            case SEASON_AUTUMN:
                if(time == TIME_SUNSET)
                    return BATTLE_TERRAIN_GRASS_AUTUMN_DUSK;
                else if(time == TIME_NIGHT)
                    return BATTLE_TERRAIN_GRASS_AUTUMN_NIGHT;
                else
                    return BATTLE_TERRAIN_GRASS_AUTUMN;
            break;
            case SEASON_WINTER:
                if(time == TIME_SUNSET)
                    return BATTLE_TERRAIN_GRASS_WINTER_DUSK;
                else if(time == TIME_NIGHT)
                    return BATTLE_TERRAIN_GRASS_WINTER_NIGHT;
                else
                    return BATTLE_TERRAIN_GRASS_WINTER;
            break;
        }
    }
...
static u8 GetSumOfEnemyPartyLevel(u16 opponentId, u8 numMons)
{
    ...
    u8 sum;
    u32 count = numMons;
    u8 partySize = gTrainers[opponentId].partySize;
